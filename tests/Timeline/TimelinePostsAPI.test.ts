...
describe('TimelinePostsAPI', () => {
  ...
  it('returns the list of posts on success', async () => {
    mockServer.interceptGet(`/users/user-id/timeline`, HttpResponse.json([
      post("uuid-2", "followee-id"),
      post("uuid-1", "user-id")
    ]));

    const timeline = await API.retrieveTimeline("user-id");
  ...
  it('throws USER_NOT_FOUND error when status code is 404', async () => {
    mockServer.interceptGet(`/users/any/timeline`, notFoundResponse());

    await expect(async () => {
      await API.retrieveTimeline("any");
    }).rejects.toThrow("USER_NOT_FOUND");
  });

  it('throws Network Error when Server is not reachable', async () => {
    mockServer.interceptGet('/users/any/timeline', HttpResponse.error());
